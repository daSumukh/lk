/*
  this file supports creation and deletion of multiple threads using 'Start nameofyourchoice'
  you can list currently ongoing threads using 'List'
  and the 'Ctrl+C' will kill the oldest thread created by user
  you can kill any thread using command 'Kill thread_no', you can find the thread_no using the 'List' command
  keep all other files as it while using this (eg thread.c, thread.h etc...)
*/
/*
...
volatile bool mockThread_should_exit[MAX_STRINGS] = {false};
...
int findStringIndex(const char *target) {
    for (int i = 0; i < MAX_STRINGS; ++i) {
        if (strcmp(strings[i], target) == 0) {
            return i;  // String found, return the index
        }
    }
    return -1;  // String not found, return -1 as an indicator
}

static int mockThread_entryy(void *args) {
    // Thread's main execution code
    // printf("")
    thread_t *current_thread = get_current_thread();
    int my_tno = findStringIndex(current_thread->name);

    while (!mockThread_should_exit[my_tno]) {
        int i = 0;
        while (i < 999999) {
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        for (int j = 0; j < 10000; ++j) {
            for (int k = 0; k < 100000; ++k) {
                // Some dummy complex computation
                int result = 0;
                for (int m = 1; m <= 100; ++m) {
                    result += (i * m + j * m * k) / (k + m);
                }
            }
            printf("looped for %d\n", j);
        }
...
void startMockThread(const char *thread_name) {
        ...
        printf("%s created\n", thread_name);
        mockThread_should_exit[thread_no] = false;
...
void kill_thread_no(const char *thread_No){
    int t_no = atoi(thread_No);
    strcat(strings[t_no], "  ~|DEAD|~");
    mockThread[t_no]->flags++;  // To free up stack in thread_exit
    mockThread_should_exit[t_no] = true;

    printf("thread_kill initiated for: %s\n", mockThread[t_no]->name);
}
        ...
        if (escape_level == 0) {
            switch (c) {
                ...
                case 0x3: // case for listening Ctrl+C and terminating the thread
                    ...
                    if(thread_no == thread_head)
                        break;
                    strcat(strings[thread_head], " ~|DEAD|~");
                    // mockThread[thread_head]->flags++;  // To free up stack in thread_exit
                    mockThread_should_exit[thread_head] = true;

                    mockThread[0]->state = THREAD_DEATH;
                    // mockThread[0]->queue_node.prev = 0;
                    // mockThread[0]->queue_node.next = 0; //  new way:
                    list_clear_node(&mockThread[0]->queue_node);
                    // thread_yield_2(mockThread[0]);

                    // list_delete(&mockThread[0]->thread_list_node);

                    thread_detach(mockThread[0]);

                    // mockThread[0]->state = THREAD_SUSPENDED;

                    // list_remove_head(&mockThread[0]->queue_node);
...
//          dprintf("%d: '%s'\n", i, args[i].str);
        ...
        // create mockThread on entering keywords "st" (start thread)
        ...
        if (strcmp(args[0].str, "Kill") == 0) {
            kill_thread_no(args[1].str);
        }
        if (!command) {
            if (showprompt)
